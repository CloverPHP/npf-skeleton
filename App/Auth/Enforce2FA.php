 ...
 * @package App\Auth
 */
final class Enforce2FA extends Base
{
    /**
     * GetClass
     * @param App $app
    ...
    final public function __invoke(App &$app, Module &$module)
    {
        ...
        switch ($action) {
            ...
            case 'activate2FA':
                ...
                $module->Admin->Auth->enforce2FA($id, $app->request->get('code'));
                $app->redirect("{$this->basePath}/");
                break;

            default:
                if(!$admin = $this->module->Admin->Manager->get($id))
                    $app->redirect("{$this->basePath}/");
                $app->response->add('loginAdmin', $admin);
                break;
        }
    }
}